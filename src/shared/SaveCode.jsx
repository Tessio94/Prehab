...
import Djoni from "../components/Djoni";
import emailjs from "@emailjs/browser";

const Kontakt = () => {
  const [lokacija, setLokacija] = useState(true);

  const initialValues = { user: "", email: "", location: "", poruka: "" };
  ...
  function handleSubmit(e) {
    ...
    emailjs
      .sendForm("service_gicejgd", "template_jfelqgo", form.current, {
        publicKey: "1lsKxaRsIctePGoXG",
      })
      .then(
        () => {
  ...
  const validate = (v) => {
    ...
    if (!v.location) {
      errors.location = "Ovo polje ne može ostati prazno";
    }
    return errors;
  };
    ...
    <section id="kontakt">
      ...
      <div className="flex flex-col justify-between items-center  sm:gap-0 sm:flex-row w-[95%] sm:w-[90%] md:w-[85%] lg:w-[80%] xl:w-[75%] h-[70vh] sm:h-fit  m-auto mb-[7rem] px-10 sm:px-0">
        ...
        <div className="flex flex-col gap-3 w-full  items-center justify-center border-b-2  sm:border-b-0 sm:border-r-2 border-slate-500  h-[10rem] basis-[33.3%] py-5 sm:py-0">
          <FaLocationDot />
          <p className="text-3xl sm:text-2xl md:text-3xl font-oswald">Adresa</p>
        ...
        <div className="flex flex-col gap-3 w-full items-center justify-center h-[10rem] basis-[33.3%] pt-10 sm:pt-0">
          <MdEmail />
          <p className="text-3xl sm:text-2xl md:text-3xl font-oswald">E-mail</p>
          <p className="text-xl sm:text-base md:text-xl font-oswald mt-auto">
      ...
      {/* <div className=" w-full">
        <img
          src="/public/images/zadar/letak 1png.png"
          alt=""
          className="w-full"
        />
      </div> */}
      <div className="w-full h-[800px] xl:h-fit  mt-12 flex flex-col-reverse sm:flex-row items-center justify-center gap-20 bg-slate-300 pt-16 pb-10  px-10">
        <div className="sm:w-[40%] h-full xl:h-fit flex flex-col justify-between rounded-xl overflow-hidden gap-10">
          <div className="flex flex-col justify-between items-stretch h-full xl:flex xl:flex-row xl:justify-between">
            <img
              src="/images/akcije/besplatni pregled za djecu.jpg"
              className="rounded-xl w-[70%] xl:max-w-[48%] xl:h-[auto]"
            />
            <img
          ...
          <div className="hidden xl:block">
            <img src="/images/zadar/letak 1png.png" className="rounded-xl" />
          </div>
        </div>
        <div className="relative w-[100%] sm:w-[55%] md:w-[40%]">
          <form
            className="flex flex-col gap-7"
            ref={form}
            onSubmit={handleSubmit}
          >
            <h6 className="absolute text-3xl font-oswald mb-3 underline text-red_preh font-bold text-center top-[-30px] left-[50%] translate-x-[-50%]">
              Ispunite formu
            </h6>
            {Object.keys(formErrors).length === 0 && isSubmit && (
              <p className="text-xl py-2 px-5 bg-green-500 text-slate-950 font-oswald rounded-xl">
                Uspjeh
              </p>
            )}
            ...
            {formErrors.user && (
              <p className="text-red-500 font-oswald">{formErrors.user}</p>
            )}
            <div className="flex flex-col gap-2">
            ...
            {formErrors.email && (
              <p className="text-red-500 font-oswald">{formErrors.email}</p>
            )}
            <div className="flex flex-col gap-2">
            ...
            {formErrors.location && (
              <p className="text-red-500 font-oswald">{formErrors.location}</p>
            )}
            <div className="flex flex-col gap-2">
              <label htmlFor="poruka" className="font-oswald text-xl">
                Vaš zahtjev
              </label>
              <textarea
                name="poruka"
